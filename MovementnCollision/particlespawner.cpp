...
void particleSpawner::init(QOpenGLShaderProgram *prog, int dimension){
...
void particleSpawner::genParticle(){
    ...
    //Make a 1 dim spring system (rope)
        ...
        float x = 0.0; float y = 0.7;
        for (unsigned int i = 0; i < lenght; i++){
            QVector3D fcolor = ((float)i/(float)lenght)*color;
            float z = i*spacing;
            QVector3D position = QVector3D(x, y, z);
            Particle *p = new Particle(position, radius, fcolor, velocity, program);
            particles.push_back(p);
        }
    }
